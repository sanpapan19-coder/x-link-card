...
- `get_card_click_counts` 関数 (期間別のカードクリック集計)
- `idx_click_logs_clicked_at_card_id` インデックス (期間集計の高速化)

既存のSupabaseプロジェクトを更新する場合も、同じSQLを再実行できます。テーブルとインデックスは `if not exists`、集計関数は `create or replace` で定義されています。
...
- カード一覧では「直近24時間」「直近7日」「直近30日」「全期間」を切り替えられます。期間は画面を開いた時刻から遡るローリング集計です。
- 選択期間のクリック数が多い順、または登録日が新しい順に並べ替えられます。
- Supabase利用時は `get_card_click_counts` がデータベース内でカード別件数を集計し、生ログ全件をアプリへ転送しません。
